...
def meeting(rooms, need)
  ...
  result = []
  available = rooms.map{ |c| (c[1] - c[0].length) > 0 ? c[1] - c[0].length : 0 }
  return "Not enough!" if need > available.sum
  available.each do |a|
    result.push(a) if result.sum < need
  end
  result[-1] = need - (result [0..-2]).sum
  result
end
